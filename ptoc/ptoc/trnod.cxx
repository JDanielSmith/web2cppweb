...
static proc_tp* curr_proc = nullptr;
...
void unit_node::attrib(int)
{
	...
	for (dcl = unit_decl; dcl != nullptr; dcl = dcl->next) {
		dcl->attrib(ctx_module);
	}
	for (dcl = unit_def; dcl != nullptr; dcl = dcl->next) {
		dcl->attrib(ctx_module);
	}
	if (initializer) {
		curr_proc = main = new proc_tp;
...
{
	for (decl_node* dcl = decls; dcl != nullptr; dcl = dcl->next) {
		dcl->attrib(ctx_module);
	}
}
...
static void push_with_context(b_ring* block, expr_node* ptr, stmt_node* body)
{
	...
	if (type->tag == tp_record || type->tag == tp_object) {
		for (bpp = &b_ring::curr_b_ring; *bpp != nullptr; bpp = &(*bpp)->outer) {
			if (*bpp == type) {
				*bpp = (*bpp)->outer;
				break;
			}
		...
		if (ptr->tag == tn_atom
			&& (((atom_expr_node*)ptr)->var == nullptr
...
void with_node::translate(int ctx)
{
	...
	if (ptrs->tag == tn_atom && ptrs->next == nullptr
		&& (((atom_expr_node*)ptrs)->var == nullptr
	...
	for (expr_node *e = ptrs; e != nullptr; e = e->next) {
		e->translate(language_c ? ctx_access : ctx_value);

		if (e->tag == tn_atom
			&& (((atom_expr_node*)e)->var == nullptr
				|| ((atom_expr_node*)e)->var->ring->scope != b_ring::record))
		{
			...
			if (e->type->name == nullptr && e->type->tpd != nullptr) {
...
void write_node::translate(int)
{
	...
	if (language_c) {
		if (params) {
			...
			write_format = nullptr;
			if (prm->type->tag == tp_file) {
				prm->translate(ctx_value);
				expr_node* file = prm;
				while ((prm = (write_param_node*)prm->next) != nullptr) {
					prm->translate(ctx_value);
...
{
	for (stmt_node* stmt = body; stmt != nullptr; stmt = stmt->next) {
...
{
	for (expr_node* e = list; e != nullptr; e = e->next) {
		e->attrib(ctx);
	}
	stmt->attrib(ctx);
	if (turbo_pascal) {
		for (stmt_node* st = stmt->next; st != nullptr; st = st->next) {
			st->attrib(ctx);
		}
	}
}
...
void switch_node::attrib(int)
{
	...
	for (case_node *c = cases; c != nullptr; c = c->next) {
...
void for_node::translate(int ctx)
{
	...
	if (t_to->name->tag == TKN_TO) {
		if (var && var->type->tag == tp_enum && var->type->name != nullptr) {
...
{
	bool body_is_block =
		body != nullptr && body->is_compound() && body->next == nullptr;
...
void empty_node::translate(int)
{
	...
	if (prev->out_text == nullptr || strcmp(prev->out_text, "}") != 0) {
		force_semicolon();
	}
	l_tkn->set_pos(last);
}
			...
			if (type->tag == tp_proc) {
				if (ctx == ctx_lvalue) {
					...
					if (ctx == ctx_procptr) {
						...
						type = prc->res_type;
						if (language_c && type != nullptr && type->tag == tp_array
							&& ctx != ctx_rvalue)
						{
...
void atom_expr_node::translate(int ctx)
{
	...
	if (turbo_pascal && tkn->name->tag == TKN_SELF) {
		...
		if (curr_proc && curr_proc->res_type) {
			if (curr_proc->proc_name != nullptr) {
...
void string_node::attrib(int)
{
	...
	if ((s[0] == '\'' && s[1] != '\'' && s[2] == '\'' && s[3] == '\0') ||
		...
		s[0] == '#' && strchr(s + 1, '#') == nullptr && strchr(s + 1, '\'') == nullptr)
	{
		type = &char_type;
	}
	else {
...
void string_node::translate(int ctx)
{
	...
	else if (s[0] == '#' && strchr(s + 1, '#') == nullptr
		&& strchr(s + 1, '\'') == nullptr)
	{
		tag = tn_char;
		if (s[1] == '$') {
...
{
	for (set_item_node* item = items; item != nullptr; item = item->next) {
...
void set_node::translate(int)
{
	...
	if (language_c) {
		...
		if (((set_tp*)type)->elem_type->tag == tp_enum) {
			assert(((set_tp*)type)->elem_type->name != nullptr);
...
void idx_expr_node::attrib(int ctx)
{
	...
	for (expr_node* e = indices; e != nullptr; e = e->next) {
...
void idx_expr_node::translate(int ctx)
{
	...
	for (expr_node* e = indices; e != nullptr; e = e->next) {
		...
		if (arr_type == nullptr || !arr_type->is_array()) {
		...
		if (language_c && arr_type && arr_type->is_array()) {
			if (arr_type->base != 0 && !no_index_decrement) {
				...
				{
					assert(arr_type->low_expr != nullptr);
...
void deref_expr_node::translate(int ctx)
{
	...
	l_tkn = op;
	op->disable();
	if (ptr->type != nullptr && (ptr->type->tag == tp_file || ptr->type->tag == tp_text)) {
		tag = tn_filevar;
		if (ctx != ctx_lvalue) {
			if (language_c) {
				ptr->l_tkn->append(")");
...
void access_expr_node::attrib(int)
{
	...
	if (rec->type != nullptr && (rec->type->tag == tp_record || rec->type->tag == tp_object || rec->type->tag == tp_unit)) {
...
void access_expr_node::translate(int ctx)
{
	...
	else if (rec->type != nullptr && rec->type->tag == tp_unit) {
		pnt->set_trans("::");
	}
...
void op_node::attrib(int)
{
	...
	if ((left == nullptr || (left->flags & tn_is_const)) &&
		(right == nullptr || (right->flags & tn_is_const)))
	{
		switch (tag) {
		case tn_add:
...
void fcall_node::attrib(int ctx)
{
	if (fptr->tag == tn_atom && ((atom_expr_node*)fptr)->tkn->tag != TKN_IDENT)
	{
		...
		case TKN_NEW:
			...
			if (turbo_pascal && args->next != nullptr) {
		...
		default:
			if (args != nullptr) {
				for (expr_node* expr = args->next; expr != nullptr; expr = expr->next)
				{
...
normal_call:
	...
	if (fptr->type != nullptr) {
		if (fptr->type->tag == tp_proc) {
			...
			for (expr_node* e = args; e != nullptr; e = e->next) {
				if (p != nullptr) {
			...
			if (type->tag != tp_array && type->tag != tp_record && type->tag != tp_object
				&& !(fptr->tag == tn_atom
					&& ((atom_expr_node*)fptr)->var != nullptr
...
void fcall_node::translate(int ctx)
{
	...
	if (fptr->tag == tn_atom && ((atom_expr_node*)fptr)->tkn->tag != TKN_IDENT)
	{
		...
		case TKN_NEW:
			if (language_c) {
				...
				expr_node* expr = args;
				if (expr != nullptr && expr->tag == tn_fcall
					&& expr->type != nullptr && expr->type->tag == tp_object)
				{
				...
				if (turbo_pascal) {
					if (args->type->is_reference()) {
						...
						char* cls_name;
						if (cls != nullptr && cls->name != nullptr) {
							cls_name = cls->name;
						}
						else {
	...
	if (fptr->type && fptr->type->tag == tp_proc) {
		proc_tp* prc = (proc_tp*)fptr->type->get_typedef();
		param_spec* p = prc->params;
		tpexpr* prev_param_type = nullptr;
		...
		for (expr_node* e = args; e != nullptr; e = e->next) {
			if (p != nullptr) {
				...
				if (p->var->type->is_array() && e->type != nullptr && e->type->tag == tp_char) {
					if (e->tag == tn_char) {
		...
		if (language_c) {
			...
			if (type != nullptr && type->tag == tp_array) {
				if (ctx == ctx_rvalue) {
					f_tkn->set_pos(assign_node::stmt->lval->f_tkn);
	...
	else if (type != nullptr && (type->tag == tp_record || type->tag == tp_object
		|| (type->tag == tp_array
			&& (language_c || ctx == ctx_constant))))
	{
		if (hp_pascal) {
			...
			if (language_c || type->tag == tp_record || type->tag == tp_object) {
				...
				if (args->tag != tn_string || args->next != nullptr) {
					lpar->set_trans("{{");
					rpar->set_trans("}}");
				}
			}
			...
			if (ctx != ctx_constant) {
				assert(type->name != nullptr);

				static int n_const;
				token *t = lpar->prev;
				char* const_name = dprintf("%s_const%d", type->name, ++n_const);

				assert(global_func_decl_level != nullptr);
...
void field_init_node::attrib(tpexpr* record_type)
{
	...
	if (record_type != nullptr && record_type->tag != tp_void) {
...
void record_constant_node::attrib(int)
{
	...
	if (record_type != nullptr && (record_type->tag == tp_record || record_type->tag == tp_object)) {
		record_type = record_type->get_typedef();
	}
	for (field_init_node* val = flist; val != nullptr; val = val->next) {
...
void expr_group_node::attrib(int ctx)
{
	...
	if (type != nullptr) {
		if (type->tag == tp_record || type->tag == tp_object) {
			...
			for (expr_node* e = expr; e != nullptr; e = e->next) {
				e->type = elem_type;
				e->attrib(ctx);
			}
			return;
...
{
	for (expr_node* prm = vals; prm != nullptr; prm = prm->next)
	{
...
void write_param_node::translate(int ctx)
{
	...
	if (language_c) {
		...
		if (write_format == nullptr) return;
		n_write_params += 1;

		char fmt = '?';
		switch (val->type->tag) {
		case tp_string:
			if (width == nullptr && val->tag == tn_string) {
		...
		case tp_array:
			...
			break;
		case tp_char:
			if (width == nullptr && val->tag == tn_char) {
				write_format = dprintf("%s%s", write_format,
					make_fmt_string(((string_node*)val)->value_tkn->out_text));
				token* prev = f_tkn->prev_relevant();
				if (prev->tag == TKN_COMMA) {
...
void const_def_node::translate(int)
{
		...
		|| sym->type->tag == tp_set))
	{
		...
		{
			if (enumeration == nullptr || (sym->flags & symbol::f_static)) {
				f_tkn = ident->prepend("enum { ");
				f_tkn->pos = curr_proc == nullptr ? 0 : ident->pos;
			}
			else {
	...
	if (sym->flags & symbol::f_static) {
		...
		(new token(nullptr, TKN_BEG_SHIFT, f_tkn->line, f_tkn->pos))
			->insert_b(f_tkn);
...
void const_def_part_node::translate(int ctx)
{
	...
	if (ctx == ctx_block && curr_proc->make_all_constants_global) {
		...
		global_func_decl_level->move_region(f_tkn, l_tkn);
		global_func_decl_level->prepend("\n\n");
		(new token(nullptr, TKN_BEG_SHIFT, f_tkn->line,
...
{
	for (decl_node* tpd = types; tpd != nullptr; tpd = tpd->next) {
		tpd->attrib(ctx);
	}
...
void  var_decl_node::translate(int ctx)
{
	...
	if (ctx == ctx_valpar || ctx == ctx_varpar) {
		...
		if (language_c && tpd != nullptr && tpd->tag == tpd_node::tpd_array) {
			tpd_node* eltd = ((array_tpd_node*)tpd)->eltd;
		...
		if (language_c && tpd->tag == tpd_node::tpd_array) {
			...
			if (language_c && tpd->tag == tpd_node::tpd_ref) {
				for (token_list* tkn = vars->next; tkn != nullptr; tkn = tkn->next) {
					tkn->ident->prepend("*");
				}
			}
			f_tkn = f_tkn->prepend(" ");
...
void proc_decl_node::attrib(int ctx)
{
	...
	type = new proc_tp(ret_type ? ret_type->type : (tpexpr*)nullptr, (tpd_node*)this);
...
void proc_decl_node::insert_params() {
	...
	if (language_c && type->res_type != nullptr
	...
	for (param_spec* p = type->extra_params; p != nullptr; p = p->next) {
...
void proc_def_node::attrib(int ctx)
{
	...
	if (t_class != nullptr) { // class method
		...
		if (s_self == nullptr) {
			...
			if (var == nullptr || var->type->tag != tp_proc) {
		...
		{
			if (((proc_tp*)var->type)->forward != nullptr
				&& params == nullptr && ret_type == nullptr)
			{
				use_forward = TRUE;
				type = (proc_tp*)var->type;
			}
...
void proc_def_node::translate(int ctx)
{
	...
	if (ctx != ctx_block) {
		...
		if (type->n_subproc > 0 && t_class == nullptr) { // make forward declaration
...
void enum_tpd_node::attrib(int)
{
	...
	for (token_list* t = items; t != nullptr; t = t->next) {
...
static array_tp* curr_array = nullptr;
...
void range_index_node::translate(int)
{
	...
	if (language_c) {
		curr_array->set_dim(nullptr, nullptr, low, high);
		if (no_index_decrement) {
	...
	if (language_c) {
		...
		if (type->tag == tp_dynarray) {
			...
			if (eltd->type->tag == tp_dynarray) {
				...
				t_array->set_trans(indices->next == nullptr
			...
			if (indices->next == nullptr && eltd->tag == tpd_array && !((array_tp*)eltd->type)->elem_type->is_array()) {
				token::disable(t_rbr->next, ((array_tpd_node*)eltd)->t_lbr);
				t_array->set_trans("matrix");
				l_tkn = eltd->l_tkn;
			}
			else {
				if (indices->next != nullptr && indices->next->next == nullptr) {
					t_array->set_trans("matrix");
				}
				token::disable(t_rbr->next, eltd->f_tkn->prev);
				l_tkn = eltd->l_tkn->append(">");
...
void variant_part_node::attrib(int ctx)
{
	...
	if (language_c && ctx != ctx_union) {
		...
		for (variant_node* vp = variants; vp != nullptr; vp = vp->next) {
			vp->attrib(ctx);
		}
		struct_path = save_path;
	}
...
void variant_part_node::translate(int ctx)
{
	...
	if (selector->tag_field != nullptr) {
		assert(selector->tag_type->type->name != nullptr);
...
{
	return (var_part == nullptr && (fix_part == nullptr ||
		(fix_part->vars->next == nullptr && fix_part->next == nullptr)));
}

void field_list_node::attrib(int)
{
	ctx = (smart_union && fix_part == nullptr && var_part != nullptr
		&& var_part->selector->tag_field == nullptr)
		? ctx_union : ctx_record;

	for (decl_node *dcl = fix_part; dcl != nullptr; dcl = dcl->next) {
		dcl->attrib(ctx);
	}
	if (var_part) {
...
void field_list_node::translate(int)
{
	...
	for (decl_node *dcl = fix_part; dcl != nullptr; dcl = dcl->next) {
		dcl->translate(ctx);
		if (f_tkn == nullptr) f_tkn = dcl->f_tkn;
		l_tkn = dcl->l_tkn;
	}
	if (var_part) {
		var_part->translate(ctx);
		if (f_tkn == nullptr) f_tkn = var_part->f_tkn;
...
void object_tpd_node::attrib(int)
{
	...
	for (decl_node* dcl = fields; dcl != nullptr; dcl = dcl->next) {
...
void record_tpd_node::translate(int ctx)
{
	...
	f_tkn = t_record;
	l_tkn = t_end;
	if (smart_union && fields->fix_part == nullptr && fields->var_part != nullptr
		&& fields->var_part->selector->tag_field == nullptr)
	{
...
void record_tpd_node::assign_name()
{
	...
	if (language_c) {
		...
		if (tag == tpd_record && outer != nullptr) {
			if (outer->type->name == nullptr) {
				outer->assign_name();
			}
