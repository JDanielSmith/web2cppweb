...
static integer f1(matrix<1,10, 1,10,real>& p, matrix<1,10,1,10,real>& q, integer& i, integer& k, integer& j);
...
static integer g(void (*f)(integer x), integer& k, integer& j)
{
    integer g_result;
    if (k < 0)  g_result = j;
    else g_result = trunc(e) + -j;
    j = succ(color,red);
    f(0);
    return g_result;
}   /*g*/
...
str11 foo(conf_array<char> a, conf_array<char> b)
{
    ...
    real r;
    rec struct_;
    matrix<1,10, 1,10,real> p;
    matrix<1,10,1,10,real> q;
    file<rec> f;
    ...
    output << "bool = " << btos(bool_) << "i = " << format(i,2) << "r = " << format(r,5,3) << NL;  
    output << "bool = " << btos(bool_) << "i = " << format(i,2) << "r = " << format(r,5,3);  
    output << "bool = " << btos(bool_) << "i = " << format(i,2) << "r = " << format(r,5,3) << NL;  
    output << "bool = " << btos(bool_) << "i = " << format(i,2) << "r = " << format(r,5,3);  

    struct_.s_green.p = struct_.s_green.q;
    if (! eof()) 
       input >> i >> struct_.s_green.v >> j >> NL;
    input >> i >> struct_.s_green.v >> j;
